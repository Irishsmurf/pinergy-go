...
## 2025-05-20 - Go Error Formatting Optimization
**Learning:** In Go, replacing `fmt.Sprintf` with string concatenation and `strconv.Itoa` for error paths reduces reflection overhead, decreases heap allocations, and improves latency. We must remember to explicitly call `.String()` on types implementing `fmt.Stringer` (like `ErrorCode`) when using string concatenation.
**Action:** Use `strconv.Itoa` and string concatenation instead of `fmt.Sprintf` for simple integer-to-string formatting in performance-critical or frequent error paths.
