...
## 2025-02-12 - Go interface inline slice allocation
**Learning:** In Go, returning an inline slice declaration like `[]byte("0")` from a method that satisfies an interface (e.g. `json.Marshaler.MarshalJSON()`) causes a heap allocation on every call because the compiler cannot prove the caller won't mutate it.
**Action:** Use a pre-allocated package-level variable instead to avoid unnecessary heap allocations on the hot path.
