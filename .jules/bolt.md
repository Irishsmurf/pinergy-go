...
## 2024-06-08 - Go fmt.Sprintf vs strconv.Itoa allocation optimization
**Learning:** In Go, using `fmt.Sprintf` for simple integer-to-string conversion and formatting causes unnecessary reflection overhead and heap allocations. Profiling revealed this as a measurable performance anti-pattern within error formatting logic (`ErrorCode.String()` and `APIError.Error()`).
**Action:** Use `strconv.Itoa` and string concatenation explicitly on types implementing `fmt.Stringer` (e.g., calling `.String()` before concatenating) to eliminate reflection overhead and reduce heap allocations.
