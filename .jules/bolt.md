...
## 2024-05-19 - Go `fmt.Sprintf` reflection and allocation overhead
**Learning:** In Go, using `fmt.Sprintf` for simple string formatting and integer conversion incurs reflection overhead and causes multiple heap allocations.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` to significantly reduce heap allocations and improve execution time in simple formatting paths.
