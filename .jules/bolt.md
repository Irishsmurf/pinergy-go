...
## 2025-02-12 - Go `fmt.Sprintf` reflection overhead in hot paths
**Learning:** In Go, using `fmt.Sprintf` for simple string building introduces hidden performance bottlenecks due to reflection and unnecessary heap memory allocations, which was confirmed by benchmarking error formatting methods. While generic string formatting tips are known, profiling revealed this as a measurable anti-pattern within our specific error-handling logic.
**Action:** Replace `fmt.Sprintf` with string concatenation and `strconv.Itoa` in frequently executed error paths to avoid reflection overhead and reduce memory allocations.
