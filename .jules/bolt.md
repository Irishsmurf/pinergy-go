...
## 2025-04-18 - Go error stringification allocation optimization
**Learning:** Using `fmt.Sprintf` for frequently accessed error formatting adds significant reflection and heap allocation overhead. In addition, using a `switch` statement for stringifying sequential enum values is less efficient than a bounds-checked array lookup.
**Action:** Replace `fmt.Sprintf` with explicit string concatenation and `strconv.Itoa`. For sequential enums (iota), use a package-level array of strings and lookup by index to avoid the switch overhead and improve performance.
