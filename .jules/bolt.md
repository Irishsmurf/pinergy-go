...
## 2024-05-24 - [Optimize UnixTime MarshalJSON]
**Learning:** Returning inline slice declarations like `[]byte("0")` from MarshalJSON allocates on every call. It is functionally safe to return global pre-allocated byte slices because the standard library's `encoding/json` package copies the data and does not mutate the returned slice.
**Action:** When optimizing Go methods (like `MarshalJSON`) by returning pre-allocated package-level byte slices to eliminate allocations, include a comment warning that the returned slice is mutable.
