...
## 2024-05-21 - [Avoid inline slice declarations in interface returns]
**Learning:** Returning inline slice declarations like `[]byte("0")` from methods returning an interface (e.g., `MarshalJSON`) causes heap allocations for every call because Go cannot prove the caller won't mutate it.
**Action:** Use pre-allocated package-level variables for static slices returned through interfaces, and add a comment warning that the returned slice is mutable.
