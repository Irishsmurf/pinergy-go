...
**Learning:** In Go, returning an inline slice declaration like `[]byte("0")` from `MarshalJSON` causes a heap allocation on every single call.
**Action:** Use a pre-allocated package-level variable instead of inline slice conversions to eliminate allocations and speed up marshaling of zero values. Always include a comment warning that the returned slice is mutable.
