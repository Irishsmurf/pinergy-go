...
## 2025-02-12 - Go MarshalJSON zero value allocation
**Learning:** In Go, returning an inline slice declaration like `[]byte("\"0\"")` from a method like `MarshalJSON` causes a heap allocation on every call.
**Action:** Declare a package-level variable for the byte slice and return it instead to eliminate the allocation (with a comment warning callers that the slice is mutable).
