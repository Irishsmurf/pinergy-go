...
## 2025-02-12 - Go `MarshalJSON` literal slice allocation
**Learning:** In Go, returning an inline slice declaration like `[]byte("0")` from `MarshalJSON` causes a heap allocation on every call.
**Action:** Return a pre-allocated package-level variable instead to eliminate the allocation for zero values.
