...
## 2025-02-12 - Go `MarshalJSON` allocation optimization
**Learning:** In Go, returning an inline slice declaration like `[]byte("0")` from a method returning an interface (e.g., `MarshalJSON`) allocates memory on the heap for every call because Go cannot prove the caller won't mutate it.
**Action:** Use a pre-allocated package-level byte slice and include a comment warning that the returned slice is mutable.
