...
## 2024-05-24 - Go `String()` method performance
**Learning:** Using `fmt.Sprintf` for formatting and large `switch` statements for string conversions in `String()` methods incurs significant reflection and allocation overhead.
**Action:** Use an array lookup for mapping sequential integer constants to strings, and fallback to `strconv.Itoa` with string concatenation. This is over 10x faster and eliminates heap allocations.
