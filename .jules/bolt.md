...
## 2024-05-13 - Replace fmt.Sprintf with string concatenation and strconv
**Learning:** Using `fmt.Sprintf` in error paths and hot code paths adds reflection overhead and heap allocations.
**Action:** Replace `fmt.Sprintf` with explicit string concatenation and `strconv.Itoa` to format integers to strings where performance is critical.
