...
## 2024-05-20 - Go interface method slice allocation
**Learning:** Returning inline byte slices like `[]byte("0")` from an interface method (e.g., `MarshalJSON`) forces a heap allocation on every call, as the compiler cannot guarantee the caller won't mutate the slice.
**Action:** Use pre-allocated package-level variables for static byte slices returned from interface methods to eliminate heap allocations, adding a comment warning that the slice is mutable.
