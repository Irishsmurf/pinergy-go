...
## 2024-05-24 - [Avoid fmt.Sprintf in simple error formatting]
**Learning:** In Go, using `fmt.Sprintf` for simple string concatenations and integer formatting involves reflection and heap allocations, making it relatively slow.
**Action:** Use `strconv.Itoa` and string concatenation (`+`) in error paths or performance-critical code where possible, as it significantly reduces latency and allocations without sacrificing readability.
