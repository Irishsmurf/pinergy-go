...
func (u *UnixTime) UnmarshalJSON(b []byte) error {
	...
	// string(b) == "null" is optimized by the Go compiler to not allocate
	if len(b) == 0 || string(b) == "null" {
		u.Time = time.Time{}
		return nil
	}

	// Use unsafe.String to convert byte slice to string without heap allocation
	// since strconv.ParseInt does not mutate or retain the string argument.
...
// Pre-allocated byte slice for zero-time to avoid heap allocation on every call.
