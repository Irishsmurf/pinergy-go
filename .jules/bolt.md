...
## 2024-06-12 - Go `MarshalJSON` allocation optimization for zero values
**Learning:** In Go, returning inline slice declarations like `[]byte("0")` from methods returning an interface (e.g., `MarshalJSON`) allocates on the heap for every call because Go cannot prove the caller won't mutate it.
**Action:** Use pre-allocated package-level variables for static byte slice returns in interface methods to eliminate allocations and improve performance, while ensuring callers don't mutate them.
