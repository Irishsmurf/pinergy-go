...
## 2024-10-24 - Go `fmt.Sprintf` vs `strconv.Itoa` and string concatenation
**Learning:** In Go, using `fmt.Sprintf` for simple integer-to-string formatting and string concatenation introduces significant reflection overhead and heap allocations.
**Action:** Replace `fmt.Sprintf` with `strconv.Itoa` and string concatenation (`+`) to reduce latency by ~60% and eliminate unnecessary allocations in error paths and core internal operations.
